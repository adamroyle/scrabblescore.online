...
class InGameControls extends React.Component {
  ...
  render() {
    ...
    const isCurrentWordEmpty =
      game.getCurrentTurn().isEmpty() && currentWord.value === "";
    ...
    const isBingoDisabled = ![...game.getCurrentTurn().words, currentWord].some(
      (word) => word.value.length >= 7,
    );

    const isModifierChosen = currentWord.modifiers.some(
    ...
    const isInstructionShown =
      ...
      currentWord.value !== "";
    const isFirstTurn =
      game.getCurrentTurnNumber() === 0 && game.getCurrentPlayerIndex() === 0;
    const isEndTurnDisabled =
      !isModifierChosen && !isCurrentWordEmpty && isFirstTurn;
    const props = {
      ref: this.input,
    ...
    return (
      <form
        ref={this.form}
        className={isFirstTurn ? "first-turn" : null}
        onSubmit={(e) => e.preventDefault()}
      >
        <ScrabbleInputBox {...props} />
        <div
          className={`instruction-message ${isInstructionShown ? "" : "hide"}`}
        >
          ↑ Press on a letter
        </div>
        ...
        <div className="buttons">
          ...
          <div className="row">
            ...
            <div className="col">
              ...
              />
              <label
                className={`btn bingo ${isBingoDisabled ? "disabled" : ""}`}
                htmlFor="bingoToggle"
              >
                BINGO
              </label>
