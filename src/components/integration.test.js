...
describe('Game', () => {
  ...
  const typeInputBox = (wrapper, input) =>
    wrapper.find('.scrabble-input-box input').simulate('change', { target: { value: input } })
  ...
  const getValue = (grid, moveIndex, playerIndex, wordIndex) =>
    getScoreGridCell(grid, moveIndex, playerIndex).find('tr').at(wordIndex).text()
  ...
  const getTotalCell = (grid, playerIndex) =>
    grid.find('ScoreGrid').find('tr.total-score').find('td').at(playerIndex).text()
  const getScoreGridCell = (grid, moveIndex, playerIndex) =>
    grid.find('tbody tr.turn-row').at(moveIndex).find('ScoreGridCell').at(playerIndex)
  const getWordAt = (grid, moveIndex, playerIndex, wordIndex) =>
    getScoreGridCell(grid, moveIndex, playerIndex).find('WordInTiles').at(wordIndex).props().word.value
  const getWinner = (wrapper) => wrapper.find('.winner').find('h1').text()
  const getLetterModifier = (wrapper, letterIndex) =>
    wrapper.find('WithModifierPopover').at(letterIndex).find('ScrabbleTile').prop('modifier')
