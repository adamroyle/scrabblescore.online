...
describe.skip("Nightmare tests", () => {
  it("selects players", async () => {
    ...
    const textContent = await nightmare.evaluate(
      () => document.querySelector(".scrabble-input-box").textContent,
    );
...
describe("Game", () => {
  function fillPlayers(wrapper, numOfPlayers) {
    ...
    for (let i = 0; i < numOfPlayers; i++) {
      wrapper
        .find("input")
        .at(i)
        .simulate("change", { target: { value: players[i] } });
    }
  ...
  const typeInputBox = (wrapper, input) =>
    wrapper
      .find(".scrabble-input-box input")
      .simulate("change", { target: { value: input } });
  const clickButton = (wrapper, regex) =>
    wrapper
      .find(".btn")
      .filterWhere((n) => n.text().match(regex))
      .simulate("click");
  ...
  const clickSubmitLeftovers = (wrapper) =>
    clickButton(wrapper, /submit.*leftovers/i);
  const clickBingo = (wrapper) =>
    wrapper.find("#bingoToggle").simulate("change");
  const clickLetterModifier = (wrapper, letterIndex, modifier) => {
    wrapper
      .find("WithModifierPopover")
      .at(letterIndex)
      .find("Tooltip")
      .prop("onVisibilityChange")(true);
    wrapper.update();
    wrapper
      .find("WithModifierPopover")
      .at(letterIndex)
  ...
  const getPlayerNames = (wrapper, playerIndex) =>
    wrapper.find("th.player-header").at(playerIndex).text();
  const getNumberOfPlayers = (wrapper) =>
    wrapper.find("th.player-header").length;
  const getCurrentPlayer = (wrapper) =>
    wrapper.find(".call-player-to-action").text();
  ...
  const getMoveNumber = (grid, moveIndex) =>
    grid.find("tbody tr.turn-row").at(moveIndex).find("th").text();
  const getTotal = (grid) =>
    grid.find("ScoreGrid").find("tr.total-score").find("th").text();
  ...
  const getTile = (wrapper, letterIndex) =>
    wrapper.find("ScrabbleInputBox").find("ScrabbleTile").at(letterIndex);
  ...
  const getCurrentLanguage = (wrapper) =>
    wrapper.find("ScoreKeeper").prop("language");
  ...
  const getbuttonText = (wrapper, buttonIndex) =>
    wrapper.find("button").at(buttonIndex).text();
  const checkInstructionMessage = (wrapper) =>
    wrapper.find("div.instruction-message").at(0).text();
  const checkHiddenInstructionMessage = (wrapper) =>
    wrapper.find("div.instruction-message.hide").exists();
