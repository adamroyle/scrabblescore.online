...
describe("scrabbleScore", () => {
  ...
  test("calculates French and Russian", () => {
    expect(
      scrabbleScore(
        "ложка",
        [["double-letter"], [], ["triple-letter"], [], ["triple-word"]],
        "ru",
      ),
    ).toEqual(69);
    expect(
      scrabbleScore(
        "yeux",
        [["double-letter"], ["blank"], ["triple-word"], ["triple-letter"]],
        "fr",
      ),
    ).toEqual(153);
  });
...
describe("indexesOf", () => {
  test("writes down indexes of the same values", () => {
    ...
    expect(indexesOf(["a", "a", "a", "b", "c", "a", "a", "c"], "a")).toEqual([
      0, 1, 2, 5, 6,
    ]);
...
describe("toggleModifiers", () => {
  ...
  test('take out the same modifier, when "blank" is also there', () => {
    expect(
      toggleModifiers(["blank", "double-letter"], "double-letter"),
    ).toEqual(["blank"]);
  });

  test('if no "blank" modifier, only one modifier is allowed', () => {
    expect(toggleModifiers(["double-letter"], "tripple-letter")).toEqual([
      "tripple-letter",
    ]);
  });

  test('add "blank" to modifier', () => {
    expect(toggleModifiers(["tripple-letter"], "blank")).toEqual([
      "blank",
      "tripple-letter",
    ]);
  });
  ...
  test('if "blank" modifier in an array,two modifiers allowed modifier is allowed', () => {
    expect(toggleModifiers(["blank"], "tripple-letter")).toEqual([
      "blank",
      "tripple-letter",
    ]);
  });
  ...
  test("blank can only be in the same array with one modifier", () => {
    expect(
      toggleModifiers(["blank", "double-letter"], "tripple-letter"),
    ).toEqual(["blank", "tripple-letter"]);
  });
});
