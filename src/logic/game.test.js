...
test("endTurn() changes player and turn number, and create an empty turn for the next player", () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    expect(
      game.playersTurns[game.getCurrentPlayerIndex()][
...
test("gerCurrentPlayer() is getting an array of turns of current player", () => {
  ...
  game = game.addWord(w2).endTurn();
  game = game.addWord(w1).endTurn();
  expect(game.getCurrentPlayer()).toEqual([
    {
      bingo: false,
      words: [{ modifiers: [null, null, null, null], score: 8, value: "rose" }],
    },
    {
      bingo: false,
      words: [
        { modifiers: [null, null, null, null], score: 10, value: "time" },
      ],
    },
    { bingo: false, words: [] },
  ]);
